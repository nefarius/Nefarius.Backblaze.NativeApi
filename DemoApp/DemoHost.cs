...
internal sealed class DemoHost(IBackblazeUploader uploader, IBackblazeBucketClient client, ILogger<DemoHost> logger)
    : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        B2ListFileVersionsResponse versions = await client.ListFileVersionsAsync(cancellationToken: stoppingToken);
